...
class env:
    ...
    def get_task_queue(self, time_slot):
        ...
        for j in range(len(self.Q_wait)):
            ...
            self.Q_wait_priority.append(np.mean(s_u + s_c + s_d) / s_T)  # todo np.mean求平均有问题，这里需要注意 2023/9/20
        ...
        while True:
            ...
            if is_available_Q_exe == 1 and len(self.Q_wait) != 0:
                ...
                # 初始化‘刚’卸载到执行队列中任务的状态
                self.time_state[self.Q_exe.index(offloading_task_id)] = self.task[
                    offloading_task_id - 1].rest_available_time
                self.up_state[self.Q_exe.index(offloading_task_id)] = self.task[offloading_task_id - 1].upload_data
                self.computing_state[self.Q_exe.index(offloading_task_id)] = self.task[
                    offloading_task_id - 1].computation_data
